...
**Goal:** Restyle the Profile page with Gothic Tech aesthetic — hero header, gold accents, filigree border, staggered animations, restructured cards.

**Architecture:** Pure CSS + React restyle. No store changes, no new Firestore reads. Two files touched: `Profile.jsx` for component restructuring, `index.css` for new utility classes (~60 lines).

**Tech Stack:** React 18, Tailwind CSS 3.4, plain CSS (index.css), Zustand stores (read-only)

---

### Task 1: Add CSS classes to index.css

**Files:**
- Append: `src/index.css` (after line 1025)

- [ ] **Step 1: Append new CSS classes to index.css**

Add the following after the last line of `src/index.css`:
...
.avatar-frame::before {
  ...
  inset: -3px;
  border-radius: 27px;
  background: linear-gradient(135deg, rgba(200,168,78,0.4), rgba(200,168,78,0.1), rgba(200,168,78,0.4));
  z-index: -1;
}
...
.progress-gold-fill {
  height: 100%;
  border-radius: 3px;
  background: linear-gradient(90deg, #C8A84E, #E8D4A0);
...
### Task 2: Rewrite Profile.jsx with Gothic Tech layout

**Files:**
- Modify: `src/pages/Profile.jsx` (full rewrite)

- [ ] **Step 1: Replace Profile.jsx with restyled version**
...
1. **Spec coverage:**
   - Hero header with avatar frame, gold name, title rule, badge — Task 2
   - Stats bar (4-column) — Task 2
   - Streak card with 3-column grid — Task 2
   - Achievements with gold unlocked/dim locked — Task 2
   - Skill progress with gold progress bars — Task 2
   - Filigree top border — Task 2
   - Staggered animations — Task 2 (all 7 sections with correct delays)
   - CSS classes — Task 1
   ✅ Full coverage

2. **Placeholder scan:** All code blocks are complete. No TBD, TODO, or "implement later". ✅

3. **Type consistency:** Only one component file, one CSS file. No cross-task type mismatches. ✅

4. **Accuracy computation:** The spec says to surface accuracy. The code computes it per-skill from `p.totalCorrect / (p.totalCorrect + wrongQuestions.length)`. This matches the data stored in `useProgressStore`. ✅
